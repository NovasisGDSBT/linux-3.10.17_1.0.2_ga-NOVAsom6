...
static struct fb_videomode ldb_modedb[] = {
	...
	/* 640x480 @ 60 Hz, 31.5 kHz hsync */
        "LDB-NOVA-640x480", 60, 640, 480, 39721, 
	48, 16, 
	33, 10, 
	96, 2, 
	0,
	...
	FB_MODE_IS_DETAILED,
	/*FB_VMODE_ASPECT_4_3, FB_MODE_IS_VESA,*/
	},
	{
        /* 800x480 @ 60 Hz , pixel clk @ 30MHz */
         "LDB-NOVA-800x480", 60, 800, 480, 30303,
         124, 124,
         11, 11,
         5, 3,
         0,
				...
				struct fsl_mxc_ldb_platform_data *plat_data)
{
	...
	if (err) {
		printk("%s : get of property mode fail %d\n",__FUNCTION__,strlen(mode));
		if ( strlen(mode) != 0 )
			printk("%s : mode %s\n",__FUNCTION__,mode);
		return err;
	}
	...
	if (err) {
		printk("%s : get of property ext_ref fail\n",__FUNCTION__);
		return err;
	}
	err = of_property_read_u32(np, "ipu_id", &ipu_id);
	if (err) {
		printk("%s : get of property ipu_id fail\n",__FUNCTION__);
		return err;
	}
	err = of_property_read_u32(np, "disp_id", &disp_id);
	if (err) {
		printk("%s : get of property disp_id fail\n",__FUNCTION__);
		return err;
	}
	err = of_property_read_u32(np, "sec_ipu_id", &sec_ipu_id);
	if (err) {
		printk("%s : get of property sec_ipu_id fail\n",__FUNCTION__);
		return err;
	}
	err = of_property_read_u32(np, "sec_disp_id", &sec_disp_id);
	if (err) {
		printk("%s : get of property sec_disp_id fail\n",__FUNCTION__);
		return err;
	}
	...
	struct mxc_dispdrv_setting *setting)
{
	...
	if (!ldb->inited) {
		...
		printk("%s : second time for separate mode\n",__FUNCTION__);
		if ((ldb->mode == LDB_SPL_DI0) ||
